#!/bin/bash
# Describe consumer groups using docker exec (read-only).
# Usage: kafka_consumer_group_describe.sh <group|--all>
...
TARGET="$1"
if [ -z "$TARGET" ]; then
  echo '{"status":"error","message":"group name or --all required"}'
  exit 1
fi
